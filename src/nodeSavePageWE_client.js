...
var replaceChar = "_";
var saveHTMLAudioVideo = false;//true;
var saveHTMLObjectEmbed = false;//true;
var saveHTMLImagesAll = true;
var saveCSSImagesAll, saveCSSFontsWoff, saveScripts = true;
var maxFrameDepth = 1;//5;
...
// 导出处理过的html
async function extractHTML2() {
    const resource_list = await identifyCrossFrames();
    const collected_resources = await collectResources(resource_list);
    await loadPageLoader(collected_resources);
    return htmlOutput;
}

//资源转换为编码格式
async function collectResources(resource_list) {
    const resource_response_list = {};
    for (const [i, resource] of resource_list.entries()) {
        try {
